...
public class HttpRequest {
     ...
     *
     * @param ipPort           the HTTP request will be sent to this IP port, e.g. "8080"
     *
     * @param timeoutMillis    after that time (given in milliseconds) method sentHttp will
     *                         stop waiting for a response
     *
     */
    public HttpRequest(String ipAddress, String ipPort, int timeoutMillis) {
        this.ipAddress = ipAddress;
        this.ipPort = Integer.parseInt(ipPort);
     ...
     *
     */
    public HttpRequest(String ipAddress, String ipPort, int timeoutMillis, boolean onMainThread) {
        this.ipAddress = ipAddress;
        this.ipPort = Integer.parseInt(ipPort);
        this.timeoutMillis = timeoutMillis;
    ...
    public JSONObject sendHttp(String parameters)
        ...
        if (ipPort <= 0)
            throw new IllegalArgumentException("IP port is undefined " + parameters);
        URL url = new URL("http", ipAddress, ipPort, "tv?" + parameters);
